	...
	{#if appState.sourceOfTruth}
		...
		>
			<div class="mb-3 flex items-center gap-2">
				...
				<span class="text-xs font-bold uppercase tracking-wider" style="color: var(--accent);">
			...
			<div class="mb-4 flex flex-wrap gap-2">
				{#each [
					{ count: truth.stats.notes, label: t('stats.notes'), color: 'var(--stat-1)' },
					{ count: truth.stats.highlights, label: t('stats.highlights'), color: 'var(--stat-2)' },
					{ count: truth.stats.bookmarks, label: t('stats.bookmarks'), color: 'var(--stat-3)' },
					{ count: truth.stats.tags, label: t('stats.tags'), color: 'var(--stat-4)' },
				] as stat}
					<span
			...
			<div class="flex gap-2">
				...
				<button
					...
					onclick={() => appState.restoreBackup(truth).then((ok) => ok && appState.openExplorer(appState.backups[appState.backups.length - 1]!.archive.database, truth.deviceName))}
				>
					<Eye size={14} />
					{t('explorer.explore')}
				</button>
		...
		<div class="mb-8 flex flex-col gap-2">
			{#each mergedFiles as meta}
				<div
					...
					style="background: var(--surface-1); border-color: var(--border);"
				>
					<Merge size={18} style="color: var(--accent);" />
					<div class="min-w-0 flex-1">
						<div class="truncate text-sm font-medium">{meta.deviceName}</div>
						<div class="text-xs" style="color: var(--text-tertiary);">
